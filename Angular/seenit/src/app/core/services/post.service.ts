...
export class PostService {
  ...
  getAll() {
    return this.http.get<PostInfoModel[]>(this.ALL_POSTS);
  }
  ...
  getById(id: string) {
    return this.http.get<PostInfoModel>(this.CREATE_POST + `/${id}`);
  }
  ...
  getUserPosts() {
    return this.http
      .get<PostInfoModel[]>(`${this.BASE_URL}/posts?query={"author":"${localStorage.getItem('username')}"}&sort={"_kmd.ect": -1}`, {
